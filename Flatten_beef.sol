...
contract BEEF is ERC20Burnable, Ownable2Step, ReentrancyGuard {
    ...
    uint16 private constant FEES_BUY = 80;
    uint16 private constant FEES_SELL = 80;

    bool public start = false;

    uint128 private constant BERAinWEI = 1 * 10 ** 18;
    ...
    event Price(uint256 time, uint256 price, uint256 volumeInBera);
    ...
    function mint(address to, uint256 value) private {
        ...
        require(totalMinted <= maxSupply, "NO MORE BEEFS");
    ...
    function leverageFee(
        ...
        uint256 mintFee = (bera * buy_fee_leverage) / FEE_BASE_1000;

        uint256 interest = getInterestFee(bera, numberOfDays);
    ...
    function leverage(
        ...
        uint256 beraFee = leverageFee(bera, numberOfDays);

        uint256 userBera = bera - beraFee;

        uint256 feeAddressAmount = (beraFee * 3) / 10;
        uint256 userBorrow = (userBera * 99) / 100;
        uint256 overCollateralizationAmount = (userBera) / 100;
        uint256 subValue = feeAddressAmount + overCollateralizationAmount;
        uint256 totalFee = (beraFee + overCollateralizationAmount);
        uint256 feeOverage;
        if (msg.value > totalFee) {
            feeOverage = msg.value - totalFee;
            sendBera(msg.sender, feeOverage);
        }
        require(
            msg.value - feeOverage == totalFee,
            "Insufficient bera fee sent"
        );

        // AUDIT: to user round down
        uint256 userBeefs = BERAtoBEEFSLev(userBera, subValue);
        mint(address(this), userBeefs);

        require(feeAddressAmount > MIN, "Fees must be higher than min.");
        sendBera(FEE_ADDRESS, feeAddressAmount);

        addLoansByDate(userBorrow, userBeefs, endDate);
        Loans[msg.sender] = Loan({
            collateral: userBeefs,
            borrowed: userBorrow,
            endDate: endDate,
    ...
    function borrow(uint256 bera, uint256 numberOfDays) public nonReentrant {
        require(
            numberOfDays < 366,
            "Max borrow/extension must be 365 days or less"
        );
        require(bera != 0, "Must borrow more than 0");
        if (isLoanExpired(msg.sender)) {
            delete Loans[msg.sender];
        }
        require(
        ...
        uint256 beraFee = getInterestFee(bera, numberOfDays);

        uint256 feeAddressFee = (beraFee * 3) / 10;

        //AUDIT: beefs required from user round up?
        uint256 userBeefs = BERAtoBEEFSNoTradeCeil(bera);

        uint256 newUserBorrow = (bera * 99) / 100;
        ...
        _transfer(msg.sender, address(this), userBeefs);
        ...
        addLoansByDate(newUserBorrow, userBeefs, endDate);

        safetyCheck(beraFee);
    }
    function borrowMore(uint256 bera) public nonReentrant {
        ...
        uint256 beraFee = getInterestFee(bera, newBorrowLength);

        //AUDIT: beefs required from user round up?
        uint256 userBeefs = BERAtoBEEFSNoTradeCeil(bera);
        uint256 userBorrowedInBeefs = BERAtoBEEFSNoTrade(userBorrowed);
        uint256 userExcessInBeefs = ((userCollateral) * 99) /
            100 -
            userBorrowedInBeefs;

        uint256 requireCollateralFromUser = userBeefs;
        if (userExcessInBeefs >= userBeefs) {
    ...
    function removeCollateral(uint256 amount) public nonReentrant {
        ...
        require(
            Loans[msg.sender].borrowed <=
                (BEEFStoBERA(collateral - amount) * 99) / 100,
    ...
    function flashClosePosition() public nonReentrant {
        ...
        uint256 collateralInBera = BEEFStoBERA(collateral);
        _burn(address(this), collateral);

        uint256 collateralInBeraAfterFee = (collateralInBera * 99) / 100;

        uint256 fee = collateralInBera / 100;
        require(
            collateralInBeraAfterFee >= borrowed,
            "You do not have enough collateral to close position"
        );

        uint256 toUser = collateralInBeraAfterFee - borrowed;
    ...
    function safetyCheck(uint256 bera) private {
        ...
        require(
            ...
            "The beefs balance of the contract must be greater than or equal to the collateral"
        );
        require(lastPrice <= newPrice, "The price of beefs cannot decrease");
        lastPrice = newPrice;
        emit Price(block.timestamp, newPrice, bera);
    }

    function BEEFStoBERA(uint256 value) public view returns (uint256) {
        return Math.mulDiv(value, getBacking(), totalSupply());
    }

    function BERAtoBEEFS(uint256 value) public view returns (uint256) {
        return Math.mulDiv(value, totalSupply(), getBacking() - value);
    }

    function BERAtoBEEFSLev(
        uint256 value,
        uint256 fee
    ) public view returns (uint256) {
    ...
    function BERAtoBEEFSNoTradeCeil(
        uint256 value
    ) public view returns (uint256) {
        uint256 backing = getBacking();
        return (value * totalSupply() + (backing - 1)) / backing;
    }
    function BERAtoBEEFSNoTrade(uint256 value) public view returns (uint256) {
        uint256 backing = getBacking();
        return Math.mulDiv(value, totalSupply(), backing);
    }

    function sendBera(address _address, uint256 _value) internal {
        (bool success, ) = _address.call{value: _value}("");
        require(success, "BERA Transfer failed.");
        emit SendBera(_address, _value);
    }

    //utils
    function getBuyBeefs(uint256 amount) external view returns (uint256) {
        return
    ...
    function getSellAmount(uint256 beefs) external view returns (uint256) {
        uint256 bera = BEEFStoBERA(beefs);
        uint256 beraAfterFee = (bera * sell_fee) / FEE_BASE_1000;
        return beraAfterFee;
    }
    ...
    function disableWhitelistRequirement() external onlyOwner {
        require(isWhitelistRequired, "Already disabled"); // Prevent redundant calls
